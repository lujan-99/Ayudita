<x-admin-layout title="Editar Docente" headerText="Módulo de Docentes Universitarios">
    <div class="mb-6">
        ...
        <h2 class="font-display text-headline-md font-bold text-on-surface">Editar Docente</h2>
        <p class="font-body-sm text-on-surface-variant">Modifica los detalles, foto y calificación del docente seleccionado.</p>
    </div>

    <!-- Form Card -->
    <div class="glass-panel rounded-lg p-6 max-w-lg">
        <form action="{{ route('admin.docentes.update', $docente) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <!-- Foto de Perfil -->
            <div>
                <label class="block text-body-sm font-medium text-on-surface mb-2">Foto de Perfil</label>
                @if($docente->foto)
                    <div class="mb-3 flex items-center gap-4 p-3 bg-surface-container-low border border-outline-variant rounded-lg">
                        <img src="{{ asset($docente->foto) }}" alt="{{ $docente->nombre_completo }}" class="w-16 h-16 rounded-full object-cover border border-outline-variant">
                        <div>
                            <p class="text-xs text-on-surface font-semibold">Foto actual registrada</p>
                            <p class="text-[10px] text-on-surface-variant">Sube un nuevo archivo si deseas reemplazarla.</p>
                        </div>
                    </div>
                @endif
                <input
            ...
            <div>
                ...
                <div class="input-focus-ring relative rounded-lg border border-outline-variant bg-surface transition-all">
                    ...
                    <input
                        ...
                        max="5"
                        name="calificacion"
                        value="{{ old('calificacion', $docente->calificacion) }}"
                        placeholder="Ej. 4.5"
                        required
                    >
                </div>
            ...
            <div>
                ...
                >{{ old('detalles_basicos', $docente->detalles_basicos) }}</textarea>
