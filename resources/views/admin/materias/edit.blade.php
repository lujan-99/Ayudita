<x-admin-layout title="Editar Materia" headerText="Módulo de Materias y Asignaturas">
    <div class="mb-6">
        ...
        <h2 class="font-display text-headline-md font-bold text-on-surface">Editar Materia</h2>
        <p class="font-body-sm text-on-surface-variant">Modifica los detalles de la asignatura seleccionada.</p>
    </div>

    <!-- Form Card -->
    <div class="glass-panel rounded-lg p-6 max-w-lg">
        <form action="{{ route('admin.materias.update', $materia) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="input-focus-ring relative rounded-lg border border-outline-variant bg-surface transition-all">
                    ...
                    <select
                        ...
                        required
                    >
                        <option value="" disabled class="bg-surface-container text-on-surface">Selecciona la Carrera</option>
                        @foreach($carreras as $carrera)
                            <option value="{{ $carrera->id }}" {{ old('carrera_id', $materia->carrera_id) == $carrera->id ? 'selected' : '' }} class="bg-surface-container text-on-surface">
                                {{ $carrera->nombre }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <div class="input-focus-ring relative rounded-lg border border-outline-variant bg-surface transition-all">
                    ...
                    <select
                        ...
                        required
                    >
                        <option value="" disabled class="bg-surface-container text-on-surface">Selecciona el Semestre</option>
                        @for($i = 1; $i <= 12; $i++)
                            <option value="{{ $i }}" {{ old('semestre', $materia->semestre) == $i ? 'selected' : '' }} class="bg-surface-container text-on-surface">
                                {{ $i }}° Semestre
                            </option>
                        @endfor
                    </select>
