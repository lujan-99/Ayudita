<x-guest-layout title="Iniciar Sesión">
    ...
    <x-auth-session-status class="mb-4 text-center text-error" :status="session('status')" />
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div class="flex items-center justify-between font-body-sm text-body-sm">
            <label for="remember_me" class="flex items-center">
                <input
                    id="remember_me"
                    type="checkbox"
                    class="h-4 w-4 rounded border-outline-variant bg-surface text-primary focus:ring-primary focus:ring-offset-background"
                    name="remember"
                >
            ...
            @if (Route::has('password.request'))
                <a class="font-medium text-primary transition-colors hover:text-primary-container" href="{{ route('password.request') }}">
        ...
        <div>
            <button type="submit" class="auth-submit-btn font-body-lg text-body-lg">
                Iniciar Sesión
            </button>
        </div>
    </form>
    ...
    <div class="mt-6">
        ...
        <div class="mt-6">
            <button
                class="inline-flex w-full items-center justify-center rounded-lg border border-outline-variant bg-surface px-4 py-3 font-body-sm text-body-sm text-on-surface transition-colors hover:bg-surface-variant"
                type="button"
            >
    ...
    <div class="mt-8 text-center font-body-sm text-body-sm">
        ...
        @if (Route::has('register'))
            <a class="ml-1 font-medium text-primary transition-colors hover:text-primary-container" href="{{ route('register') }}">
                Regístrate gratis
            </a>
        @endif
    </div>
